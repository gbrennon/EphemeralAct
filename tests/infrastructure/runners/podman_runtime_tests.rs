...
fn new_connects_to_socket() {
    assert!(PodmanRuntime::new().is_ok());
}
...
fn copy_from_reads_file_from_container() {
    ...
    // Create a file inside the container first
    container
        .exec(
            &[
                "sh".into(),
...
fn remove_with_force_cleans_up_container() {
    ...
    // Remove should succeed even while container is running (force)
    container.remove().unwrap();

    // Second remove on same container handle should still be a no-op
