...
use ephact::{
    application::{
        ...
        services::build_job_environment_service::BuildJobEnvironmentService,
    },
...
fn execute_lets_the_job_environment_override_the_workflow_one() {
    ...
    let env = BuildJobEnvironmentService::new().execute(BuildJobEnvironmentRequest {
        workflow: &workflow,
        job_env: &job_env(&[("MODE", "job")]),
    });

    assert_eq!(env.get("MODE").map(String::as_str), Some("job"));
}
...
fn execute_keeps_a_job_declared_path() {
    ...
    assert_eq!(env.get("PATH").map(String::as_str), Some("/job/bin"));
}
