...
fn execute_pulls_and_returns_the_mapped_image() {
    ...
    let service = PullJobImageService::new(runtime.clone(), Box::new(FakeImageMapper));
    ...
    assert_eq!(runtime.pulled_images.borrow().clone(), vec!["ubuntu-22.04"]);
}
...
fn execute_maps_ubuntu_latest_when_the_job_declares_no_runner() {
    ...
    assert_eq!(
        runtime.pulled_images.borrow().clone(),
        vec!["ubuntu-latest"]
    );
}
